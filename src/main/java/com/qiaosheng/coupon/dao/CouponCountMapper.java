...
public interface CouponCountMapper {
     ...
     * @return
     */
    @Select("select c.shop_id,c.shop_name,c.scan_times,nvl(c.button_count,0) as button_count,nvl(d.share_count,0) as share_count from (select a.*,b.button_count from(select shop.shop_id,shop.shop_name,shop.scan_times from SHOP shop where is_delete='0')a left join\n" +
